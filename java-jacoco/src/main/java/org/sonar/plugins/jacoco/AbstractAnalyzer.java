...
public abstract class AbstractAnalyzer {
  ...
  public final void analyse(Project project, SensorContext context) {
    ...
    if(path != null && path.startsWith("http://")) {
      JaCoCoExtensions.LOG.info("HTTP URL detected ...");
      ...
      if(documentId == null) {
        JaCoCoExtensions.LOG.error("documentId query parameter is missing from HTTP URL");
        throw new IllegalArgumentException("documentId query parameter is missing from HTTP URL");
      }

      executionDataReaderPack = executionDataReaderStrategy.readExecutionDataFromHttp(path, documentId);
    } else {
